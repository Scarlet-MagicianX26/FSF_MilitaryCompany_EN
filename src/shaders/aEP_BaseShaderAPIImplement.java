...
public class aEP_BaseShaderAPIImplement implements ShaderAPI
{
  ...
  public List<CombatLayeredRenderingPlugin> getRenderList(){
    if(!Global.getCombatEngine().getCustomData().containsKey(shaderId)) {
      List<CombatLayeredRenderingPlugin> renderList = new LinkedList<CombatLayeredRenderingPlugin>();
      Global.getCombatEngine().getCustomData().put(shaderId,renderList);
      return renderList;
    }
    return (List<CombatLayeredRenderingPlugin>)Global.getCombatEngine().getCustomData().get(shaderId);
  }
}

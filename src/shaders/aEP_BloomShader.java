...
public class aEP_BloomShader extends aEP_BaseShaderAPIImplement
{
  ...
  public static void add(CombatLayeredRenderingPlugin CVEWS) {
  ...
  public void renderInWorldCoords(ViewportAPI viewportAPI) {
     ...
     */
    List<CombatLayeredRenderingPlugin> allRenders = getRenderList();
    ...
    for (int i = 0; i < allRenders.size(); i++) {
      CombatLayeredRenderingPlugin cvf = allRenders.get(i);
      //radius < 0 是黑话，用于在不触发写在 isExpired里面的 readyToEnd的情况下检测是否已经终结
      if (cvf.getRenderRadius() < 0) {

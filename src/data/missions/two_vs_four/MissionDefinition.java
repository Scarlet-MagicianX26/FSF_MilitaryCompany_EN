...
public class MissionDefinition implements MissionDefinitionPlugin
{
  ...
  public void defineMission(MissionDefinitionAPI api) {
    ...
    api.addPlugin(new BaseEveryFrameCombatPlugin()
    {
      ...
      final List<FleetMemberAPI> playerFleet = new ArrayList<>();
      ...
      public void advance(float amount, List events) {
        ...
        if (!didOnce && !Global.getCombatEngine().getCombatUI().isShowingDeploymentDialog()) {
          ...
          didOnce = true;
          //先把没有部署的全部强制部署
          playerFleet.addAll(Global.getCombatEngine().getFleetManager(0).getReservesCopy());
          for (FleetMemberAPI reserve : playerFleet) {
          ...
          for (ShipAPI s : Global.getCombatEngine().getShips()) {
            s.getVelocity().set(new Vector2f(0f, 0f));
            if (s.getFleetMember().getShipName().equals("Lamdor Defender")) s.getLocation().set(new Vector2f(200, 200));
            if (s.getFleetMember().getShipName().equals("Ship Shredder")) s.getLocation().set(new Vector2f(-200, -200));
          }

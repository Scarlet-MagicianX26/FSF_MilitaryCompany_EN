  ...
  override fun apply(stats: MutableShipStatsAPI?, id: String, state: ShipSystemStatsScript.State, effectLevel: Float) {
    ...
    for (weapon in ship.allWeapons) {
      ...
      if (weapon.slot.id.contains("HD01")) {
        (weapon.effectPlugin as aEP_FCLAnimation).setMoveToLevel(0f)
        if (leftAmmo <= 0) (weapon.effectPlugin as aEP_FCLAnimation).setMoveToLevel(1f)
        if (oilL != null && oilL.animation.frame == 0 && (weapon.effectPlugin as aEP_FCLAnimation).decoMoveController.effectiveLevel >= 1) {
      ...
      if (weapon.slot.id.contains("HD02")) {
        (weapon.effectPlugin as aEP_FCLAnimation).setMoveToLevel(0f)
        if (leftAmmo <= 0) (weapon.effectPlugin as aEP_FCLAnimation).setMoveToLevel(1f)
        if (oilR != null && oilR.animation.frame == 0 && (weapon.effectPlugin as aEP_FCLAnimation).decoMoveController.effectiveLevel >= 1) {

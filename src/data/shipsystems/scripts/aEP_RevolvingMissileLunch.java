...
public class aEP_RevolvingMissileLunch extends BaseShipSystemScript
{
  ...
  public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
    ...
    //系统没激活从这出去
    if (effectLevel < 0.99 && !forceUse) return;
    ...
    for (WeaponAPI w : ship.getAllWeapons()) {
      if (w.getSpec().getWeaponId().equals("aEP_revo_missile")) {
        if (ship.getFluxTracker().getHardFlux() < FIRE_CONVERT_HARD_FLUX) {
        ...
        aEP_CombatEffectPlugin.Mod.addVE(new aEP_ARC_SPAWN(ship, missile));

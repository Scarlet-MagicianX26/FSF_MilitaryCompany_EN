...
class aEP_MDDroneLaunchAI:aEP_BaseSystemAI() {
  override fun initImpl() {
    thinkTracker.setInterval(0.4f,0.6f)
  }
  ...
  override fun advanceImpl(amount: Float, missileDangerDir: Vector2f?, collisionDangerDir: Vector2f?, target: ShipAPI?) {
    ...
    //遍历友军，根据危险程度加入权重选择器
    for(s in AIUtils.getNearbyAllies(ship, aEP_Tool.getSystemRange(ship,aEP_MDDroneLaunch.SYSTEM_RANGE) + ship.collisionRadius + 200f)){
    ...
    if(target != null){
      ship.setCustomData("aEP_MDDroneLaunchAI_assign_target", aEP_Tool.getExtendedLocationFromPoint(target.location,target.facing,target.collisionRadius/2f))
      ship.useSystem()
    }
  }

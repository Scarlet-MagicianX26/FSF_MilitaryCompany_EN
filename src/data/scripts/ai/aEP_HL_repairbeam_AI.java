...
import data.scripts.weapons.aEP_HLDroneBeam;
...
public class aEP_HL_repairbeam_AI implements AutofireAIPlugin
{
  ...
  public aEP_HL_repairbeam_AI(WeaponAPI weapon) {
    this.weapon = weapon;
  ...
  public void advance(float amount) {
    ...
    if (lifeTimer > INTERVAL_TIME) {
      ...
      if (aEP_Tool.isWithinArc(parentShip, weapon) && (!needRepair().isEmpty() || (parentShip.getArmorGrid().getArmorValue(minArmorGrid[0], minArmorGrid[1]) / parentShip.getArmorGrid().getMaxArmorInCell()) < aEP_HLDroneBeam.MIN_ARMOR_PERCENT)) {

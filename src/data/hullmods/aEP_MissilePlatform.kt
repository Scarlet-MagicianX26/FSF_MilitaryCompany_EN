...
class aEP_MissilePlatform : aEP_BaseHullMod() {
  ...
  companion object {
    private const val MISSILE_HITPOINT_BUFF = 15f //by percent
    private const val MISSILE_ROF_BUFF = -20f //by percent
  ...
  override fun addPostDescriptionSection(tooltip: TooltipMakerAPI, hullSize: HullSize, ship: ShipAPI, width: Float, isForModSpec: Boolean) {
    ...
    tooltip.addPara("- " + aEP_DataTool.txt("missile_reload_speed_down") + "{%s}", 5f, Color.white, Color.red, Math.abs(MISSILE_ROF_BUFF).toInt().toString() + "%")
  ...
  private inner class LoadingMap constructor(var ship: ShipAPI) : AdvanceableListener {
    ...
    override fun advance(amount: Float) {
      ...
      for (w in ship.allWeapons) {
        val slot = w.slot
        if (!slot.isBuiltIn && !slot.isDecorative && !slot.isHidden) {
